...
## Frontend
- Replaced the chart library dependency with a custom renderer (`frontend-demo/simple-chart.js`) to avoid CDN/CORS issues and keep everything self-contained.
...
- Pointed `frontend-demo/main.js` at `http://localhost:8000/candles`, normalized series data, and computed the above stats after every fetch so the chart and summary stay synchronized.
