...
def classify_regimes(
    ...
    df['RVOL'] = df['volume'] / df['volume'].rolling(rvol_len, min_periods=max(5, rvol_len//2)).mean()

    # Gap vs prior close
    df['Gap_pct'] = (df['open'] - df['close'].shift(1)) / df['close'].shift(1) * 100.0

    # --- Rolling adaptive ATR% quantiles ---
    q_lo = df['ATR_pct'].rolling(roll_quant_window, min_periods=max(20, atr_len)).quantile(vol_quantiles[0])
    q_hi = df['ATR_pct'].rolling(roll_quant_window, min_periods=max(20, atr_len)).quantile(vol_quantiles[1])
