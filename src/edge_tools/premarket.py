...
class PremarketPriceCalculator:
    def __init__(self, data: pd.DataFrame):
        ...
        if len(self._trading_dates) > 2:
            self._valid = False
            raise ValueError("Data contains more than two trading dates. Please provide data for only the selected date and previous date.")
        self._selections = {
    ...
    def compute_price(self, selection_key: str) -> float:
        ...
        try:
            price = prev_day_data.loc[prev_day_data[f"{column_name}_only"] == target_time].iloc[0]["close"]
            logger.debug(f"Computed price for {selection_key}: {price}")
        except IndexError:
            raise ValueError(f"No data found for the specified time {target_time} on {prev_date}.")
        return price
...
def compute_premarket_prices(sliced_data):
    ...
    us_close_previous_day_price = premarket_calculator.compute_price("us_close_previous_day")
